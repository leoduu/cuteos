...
impl drivers::Driver for PL011Uart {
    #[inline(always)]
...
impl console::Interface for PL011Uart {
    /// Passthrough of `args` to the `core::fmt::Write` implementation, but guarded by a Mutex to
    /// serialize access.
    #[inline(always)]
    fn write_char(&self, ch: char) {
        self.inner.lock().write_char(ch)
    }
    ...
    fn write_fmt(&self, args: core::fmt::Arguments) -> fmt::Result {
        ...
        // readability.
        fmt::write(&mut *self.inner.lock(), args)
    }
    ...
    fn read_char(&self) -> char {
        self.inner.lock().read_char_converting(BlockingMode::Blocking).unwrap()
    }
}
